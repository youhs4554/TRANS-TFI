...
class Trainer:
    ...
    def train_single_event_custom(self,
                                  train_set,
                                  val_set=None,
                                  batch_size=64,
                                  epochs=100,
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(num_train_batch):
                ...
                if len(self.metrics) == 1:  # only NLLPCHazardLoss is asigned
                    batch_loss = self.metrics[0](phi[1].sigmoid().unsqueeze(2), batch_y_train)
            ...
            history['train_loss'].append(epoch_loss / (batch_idx + 1))
            history['train_L_z'].append(self.metrics[0].L_z.item())
            history['train_L_c'].append(self.metrics[0].L_c.item())
            history['train_nll_loss'].append(self.metrics[0].nll_loss.item())
            ...
            if val_set is not None:
                ...
                history['val_L_c'].append(self.metrics[0].L_c.item())
                history['val_nll_loss'].append(self.metrics[0].nll_loss.item())
                self.early_stopping(val_loss.item(), self.model, name=self.ckpt)
                if self.early_stopping.early_stop:
                    print(f"early stops at epoch {epoch + 1}")
                    # load best checkpoint
    ...
    def fit(self,
        ...
        **kwargs,
        ...
        if self.model.config.num_event == 1:
            if self.model.config.custom_training:
                return self.train_single_event_custom(
                    train_set=train_set,
                    val_set=val_set,
